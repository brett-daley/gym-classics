...
| Env ID | Description |
| :-: | --- |
| `5Walk-v0` | A 5-state deterministic linear walk. Ideal for implementing random walk experiments. Reference: [[3]](#references) (page 125).<br><br>**states:** Discrete positions {0, ..., 4} on the number line.<br><br>**actions:** Move left/right.<br><br>**rewards:** +1 for reaching the extreme right.<br><br>**termination:** Reaching the extreme left or right. |
| `19Walk-v0` | Same as `5Walk` but with 19 states and an additional -1 reward for reaching the extreme left. Reference: [[3]](#references) (page 145). |
| `ClassicGridworld-v0` | A pedagogical 4x3 gridworld with the agent starting in the bottom-left cell. Actions are noisy; with a 10% chance each, a move action may rotated by 90 degrees clockwise or counter-clockwise. Reference: [[1]](#references) (page 646).<br><br>**states**: Grid cells.<br><br>**actions**: Move up/right/down/left.<br><br>**rewards**: +1 for taking any action in the top-right cell. -1 for taking any action in the mid-right cell.<br><br>**termination**: Earning a nonzero reward. |
| `CliffWalk-v0` | The Cliff Walking task used to contrast Sarsa and Q-Learning. The agent begins in the bottom-left cell and must navigate to the goal (bottom-right cell) without entering the region labeled "The Cliff." Reference: [[3]](#references) (page 132, example 6.6).<br><br>**states**: Grid cells.<br><br>**actions**: Move up/right/down/left.<br><br>**rewards**: -100 for entering The Cliff. -1 for all other transitions.<br><br>**termination**: Entering The Cliff or reaching the goal. |
| `DynaMaze-v0` | A 9x6 deterministic gridworld with barriers to make navigation more challenging. The agent starts on the left side at coordinates (0, 3); the goal is the top-right cell. Reference: [[3]](#references) (page 192).<br><br>**states**: Agent cell location.<br><br>**actions**: Move up/right/down/left.<br><br>**rewards**: +1 for reaching the goal.<br><br>**termination**: Reaching the goal. |
| `FourRooms-v0` | An 11x11 gridworld segmented into four rooms. The agent begins in the bottom-left cell; the goal is in the top-right cell. Actions are noisy. NOTE: Instead of the original transition probabilities, this implementation uses the 80-10-10 rule from `ClassicGridworld`. Reference: [[2]](#references) (page 192).<br><br>**states**: Grid cells.<br><br>**actions**: Move up/right/down/left.<br><br>**rewards**: +1 for reaching the goal.<br><br>**termination**: Reaching the goal. |
| `JacksCarRental-v0` | A challenging management problem where a rental company must balance the number of cars between two parking lots to maximize its profit. On each timestep, a Poisson-distributed number of requests and returns come into each lot. (The lots have different statistics.) The agent may then move up to 5 cars between the lots for a proportional fee. The lots can never have more than 20 cars each, and a lot earns money for a request only if it has a car available. Reference: [[3]](#references) (page 81, example 4.2).<br><br>**states:** The number of cars at both lots.<br><br>**actions:** Move a number of cars {-5, ..., 5}. Positive numbers represent moving cars from lot 1 to lot 2; negative numbers represent moving cars from lot 2 to lot 1.<br><br>**rewards:** +10 for each satisfied request. -2 for each car moved.<br><br>**termination:** 100 timesteps elapse. |
| `JacksCarRentalModified-v0` | Same as `JacksCarRental` but with two modifications to the reward function:<br><br>1. One of Jack's employees can move a car from lot 1 to 2 for free.<br><br>2. Overnight parking incurs -4 reward per lot with more than 10 cars.<br><br>Reference: [[3]](#references) (page 82, exercise 4.7). |
| `Racetrack1-v0` | A gridworld-type racetrack where the agent must traverse a right turn and reach the finish line as quickly as possible. The agent begins at a random location on the starting line and can only directly control the velocity of the racecar (not its position). Each velocity component can never be negative nor greater than 5. If the car goes out of bounds, it is reset to a random location on the starting line without terminating the episode. NOTE: While the original version forbids both velocity components from being zero simultaneously, no such restriction is enforced in this implementation. Reference: [[3]](#references) (page 112, figure 5.5, left).<br><br>**states:** Racecar position and velocity.<br><br>**actions:** Changes to the racecar's *velocity* (not position) vector, where the x- and y- components can be independently modified by {-1, 0, +1} on each timestep. This gives a total of 9 actions.<br><br>**rewards:** -1 on all transitions unless the finish line is reached.<br><br>**termination:** Reaching the finish line. |
| `Racetrack2-v0` | Same as `Racetrack1` but with a different track layout. Reference: [[3]](#references) (page 112, figure 5.5, right). |
| `SparseGridworld-v0` | A 10x8 featureless gridworld. The agent starts in the mid-left cell, and the goal is slightly to the right of center. To make it more challenging, the same 80-10-10 transition probabilities from `ClassicGridworld` are used. Great for testing various forms of credit assignment in the presence of noise. Reference: [[3]](#references) (page 147, figure 7.4)<br><br>**states:** Agent cell location.<br><br>**actions:** Move up/right/down/left.<br><br>**rewards:** +1 for reaching the goal.<br><br>**termination:** Reaching the goal. |
| `WindyGridworld-v0` | A 10x7 deterministic gridworld where some columns are affected by an upward wind. The agent starts in the mid-left cell, and the goal is to the right of center. If an agent executes an action from a cell with wind, the resulting position is given by the vector sum of the action's effect and the wind. Reference: [[3]](#references) (page 130, example 6.5)<br><br>**states:** Agent cell location.<br><br>**actions:** Move up/right/down/left.<br><br>**rewards:** -1 for all transitions except those reaching the goal.<br><br>**termination:** Reaching the goal. |
| `WindyGridworldKings-v0` | Same as `WindyGridworld` but with diagonal "King's" moves permitted. Reference: [[3]](#references) (page 131, exercise 6.9).<br><br>**actions:** 4 cardinal directions and 4 intermediate directions. |
| `WindyGridworldKingsNoOp-v0` | Windy Gridworld with King's moves and a no-op action. Reference: [[3]](#references) (page 131, exercise 6.9).<br><br>**actions:** 8 cardinal/intermediate directions, plus a "no-op" (do nothing) action. |
| `WindyGridworldKingsStochastic-v0` | Same as `WindyGridworldKings` but windy cells exhibit stochastic behavior: -1, +0, or +1 wind strength with probability 1/3 each. Reference: [[3]](#references) (page 131, exercise 6.10) |
